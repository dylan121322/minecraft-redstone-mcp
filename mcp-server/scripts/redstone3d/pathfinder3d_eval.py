"""
pathfinder3d_eval.py — full validation chain for the 3-D router:
    place -> PathFinder3D.route (dynamic layers + negotiation + repair)
          -> BuildableRouter._materialize -> BuildResult
...
def main():
    ...
    nl = nls[mod]
    col_gap = int(sys.argv[7]) if len(sys.argv) > 7 else 16
    pl = place(nl, col_gap=col_gap, row_gap=16)

    t0 = time.time()
    PF.RISE_COST = float(sys.argv[5]) if len(sys.argv) > 5 else 12.0
    PF.DROP_COST = float(sys.argv[6]) if len(sys.argv) > 6 else 10.0
    fmult = float(sys.argv[8]) if len(sys.argv) > 8 else 12.0
    pf = PF.PathFinder3D(pl, margin=16, max_layers=4, p_cap=128.0,
                         fanout_mult=fmult)
    placements, shorts = pf.route(max_rounds=rounds, start_layers=2)
    print(f"PathFinder3D: shorts={shorts} nets={len(placements)}/29 "
