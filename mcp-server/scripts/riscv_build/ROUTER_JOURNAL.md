...
## 十、地下 cross 平面方案（物理成立，但未提升布通率）

**动机**：地上方案的唯一失败类别是 "DESCENT conflict"——sink 侧下降楼梯要占
`depth` 格横向净空，而这些通道被先布的 net 抢占。

**验证（test_underground.py，三项全通）**
- U1 地下平面（y=-3）dust 正常导通，不受上方 y=-1 地板影响
- U2 地下→y0 的 1×1 火把塔非反相（偶数火把），信号直喂 gate 输入 pin（feed=15）
- U3 地下走线与正上方 y0 走线**完美隔离**（四种驱动组合各自独立）=全新布线容量

**实现**（stash 保留）：cross 平面移到 y0-4/-8/-12，source 侧楼梯下降（四方向搜索），
sink 侧 1×1 塔升入 pin 西侧 feed 格（零横向占用）。

**实测结果（alu1）**
| 方案 | failed | wires | 链路 |
|------|--------|-------|------|
| 地上 cross（当前最优） | 5 | 2381 | 24/24 |
| 地下 cross | 8 | 1994~3417 | 未测 |

**结论**：物理成立、wires 一度降到 1994（比地上少 16%），但**布通率没改善**。
瓶颈只是从"sink descent 走廊"转移到"source 下降楼梯 + feed 格被占"——
两侧总有一侧需要楼梯（塔只能向上），总容量约束未变。

**已证伪的三条路（勿重复）**
1. descent 走廊扩大候选（更多 z 偏移 + x 后退）→ 5 failed 变 8，jog 更长更易撞
2. LONG_HAUL 长连接上 cross → 布通 24→26 但链路仅 16/26（长 cross 电气可靠性远低于 y0）
3. 地下 cross 平面 → 见上表

**剩余 5 net（n32/n5/n4/n3/n2）的真实画像**：全是最长连接（174-257 格），
静态空间充裕（sink 西向净空 15-40 格），失败纯因先布 net 抢占。
descent hist 协商能把 failed 降到 3 但**必带短路**。
下一个可行方向：placer 层面重排 gate 列序缩短这些长连接（从源头消除 257 格跨全场连线）。
